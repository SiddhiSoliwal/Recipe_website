        ...
        <div className="flex justify-between h-16">
          ...
          >
            <Link to="/" className="flex items-center space-x-2">
              <ChefHat className="h-8 w-8 text-black-500" />
              <span className="text-2xl font-bold text-yellow-500">CulinaryDelight</span>
            </Link>
          </motion.div>

          <div className="hidden md:flex items-center space-x-8">
            {['Home', 'About', 'Recipes', 'Contact','Login'].map((item) => (
              <motion.div
                key={item}
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
